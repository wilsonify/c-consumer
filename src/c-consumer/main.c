  ...
  for (;;)
      ...
      if (now > next_summary_time)
	{
	  ...
	  printf ("%d ms: Received %d - %d since last report (%d Hz)\n",
		  (int) (now - start_time) / 1000, received,
      ...
      ret = amqp_consume_message (conn,	// state - the connection object
	  ...
	  if (AMQP_RESPONSE_LIBRARY_EXCEPTION == ret.reply_type
	      ...
	      if (AMQP_FRAME_METHOD == frame.frame_type)
		{
		  switch (frame.payload.method.id)
			...
			ret =
			  amqp_read_message (conn, frame.channel, &message,
					     0);
			if (AMQP_RESPONSE_NORMAL != ret.reply_type)
			  {
			    return;
...
main (int argc, char const *const *argv)
{
  ...
  status = amqp_socket_open (socket,	// self - A socket object.
  ...
  die_on_amqp_error (amqp_login (conn,	// state - the connection object
  ...
  amqp_exchange_declare (conn,	// state - connection state
  ...
  try_queue_struct = amqp_queue_declare (conn,	// state - connection state
					 1,	// channel - the channel to do the RPC on
  ...
  done_queue_struct = amqp_queue_declare (conn,	// state - connection state
					  1,	// channel - the channel to do the RPC on
  ...
  fail_queue_struct = amqp_queue_declare (conn,	// state - connection state
					  1,	// channel - the channel to do the RPC on
  ...
  amqp_basic_qos (conn,		// state - connection state
		  1,		// channel - the channel to do the RPC on
		  0,		// prefetch_size
  ...
  amqp_queue_bind (conn,	// state - connection state
		   1,		// channel - the channel to do the RPC on
  ...
  printf ("clean up\n");
  die_on_amqp_error (amqp_channel_close (conn,	// state - the connection object
  ...
  die_on_amqp_error (amqp_connection_close (conn,	// state - the connection object
					    AMQP_REPLY_SUCCESS	// code - the reason for closing the channel, AMQP_REPLY_SUCCESS is a good default
		     ), "Closing connection");
